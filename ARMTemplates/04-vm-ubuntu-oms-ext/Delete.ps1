...
if ( $null -eq (Get-AzVM -ResourceGroupName $ResourceGroupName -Name $VirtualMachineName -Verbose -ErrorAction SilentlyContinue) )
{
    ...
    Write-Host "`nDelete Network interface $NIC_Name."
    ...
    if($DeleteOSDisk) {
        ...
        if ( $null -eq (Get-AzDisk -ResourceGroupName $ResourceGroupName -DiskName $Os_Disk_Name -Verbose -ErrorAction SilentlyContinue) )
        {
            Write-Host "Disk with name $Os_Disk_Name is not found."
        } 
        else{
...
if($WorkSpaceName) {
    ...
    Write-Host "`nDelete Log Analytics workspace $WorkSpaceName."
    Remove-AzOperationalInsightsWorkspace   -ResourceGroupName $ResourceGroupName `
                                            -Name $WorkSpaceName `
                                            -Force `
                                            -Verbose `
                                            -ErrorVariable ErrorMessage
    ...
    if ($ErrorMessage) {
        Write-Host '', 'Deleting Log Analytics workspace failed with following errors:', @(@($ErrorMessage) | ForEach-Object { $_.Exception.Message.TrimEnd("`r`n") })
    }                                      
}